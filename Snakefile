...
rule snp_ances_file:
    ...
    run:
        ...
        anc_series = {}
        for anc in 'EUR', 'AFR', 'AMR', 'EAS', 'SAS':
            bimfile = pd.read_table(f"{IMPUTED_SPLITCHR_RACE_FILTERED_STEM}.bim".format(race=anc, chrom=wildcards.chrom), header=None)
...
rule mama:
    output:
        expand("imputed_mama_{chrom}_{race}_{phenotype}.res", race=["AMR", "AFR", "EUR"], allow_missing=True)
    input:
        expand("IMPUTED_{phenotype}_{race}.GENESIS.assoc.txt", race=["AMR", "AFR", "EUR"], allow_missing=True),
        "imputed_mama_ldscore/{chrom}.l2.ldscore.gz"
    resources:
        mem_mb=32000,
        time_min=720
    ...
    shell:
        ...
        {params.script_path} \
            --sumstats  {input[0]},AMR,{wildcards.phenotype} \
                        ...
                        {input[2]},EUR,{wildcards.phenotype} \
            --ld-scores {input[3]} \
