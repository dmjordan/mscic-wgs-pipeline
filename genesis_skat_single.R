...
gdsfile <- paste(sample_file_basename, "seq", "gds", sep=".")
...
seqFile <- seqOpen(gdsfile)
seqData <- SeqVarData(seqFile)
geneRanges <- genes(TxDb.Hsapiens.UCSC.hg38.knownGene)
geneSplit <- split(geneRanges, 1:length(geneRanges) %% 32)

run_assoc <- function (nullmod, endpoint) {

    cat("running rare variant assocation\n", file=stderr())
    assoc <- do.call(rbind, mclapply(geneSplit,                
        function (genes) { 
            assocTestSingle(
                SeqVarRangeIterator(seqData, genes),
