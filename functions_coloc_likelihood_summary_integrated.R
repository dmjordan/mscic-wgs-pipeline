# this code is adapted from coloc2
# original source: https://github.com/Stahl-Lab-MSSM/coloc2/blob/master/functions_coloc_likelihood_summary_integrated.R
...
sniff <- function(file=fname, eqtl = FALSE) {
   ...
   SNPID = which(grepl("snp|SNP|MarkerName|variant.id", names(line),  ignore.case = T))
    ...
    BETA = which(grepl("^beta$|^b$|^effect$|^or$|OR.A1|BMIadjMainEffects|^Est$", names(line),  ignore.case = T))
    ...
    PVAL = which(grepl("^p$|pval|Pvalue|P.value|P.val|BMIadjMainP|^Score\\.pval$", names(line),  ignore.case = T))[1]
    SE = which(grepl("^se|^StdErr$|BMIadjMainSE|^Est\\.SE$", names(line),  ignore.case = T))
    ...
    print(names(line))
    A1 = which(grepl("A1|Allele1|^effect\\.allele$", names(line),ignore.case=T))
    A2 = which(grepl("A2|Allele2|^other\\.allele$", names(line),ignore.case=T))
    ...
    N = which(grepl("^N$|^TotalSampleSize$|^SAMPLE_SIZE$|^n\\.obs$", names(line),  ignore.case = T))
    Ncases =  which(grepl("^N_CASES$|^N_CASE$|^Ncases$|^n\\.cases$", names(line),  ignore.case = T)) 
    Ncontrols =  which(grepl("^N_CONTROLS$|^N_CONTROL$|^Ncontrols|^n\\.controls$", names(line),  ignore.case = T)) 
...
# read sniff and quickRead functions from here: 
 ...
 #    data = quickRead(file = fname, importCols = colsAll[[1]], colClasses = colsAll[[2]], addFilter=TRUE, colToBeFiltered=colToBeFiltered, valueToBeFiltered=3)
  ...
  col.names = names(colsAll[[1]])
  data= fread(fname, select = colsAll[[1]], col.names=col.names, stringsAsFactors=FALSE)
