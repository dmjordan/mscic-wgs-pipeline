...
@click.argument("out_path", type=ClickPathlibPath())
def intersect_samples(mt_path_1, mt_path_2, out_path):
    mt_path_1 = mt_path_1.resolve()
...
def imputation_concordance(wgs_mt_path, imputed_mt_path, output_prefix):
    ...
    for label, filtered_imputed_mt in zip(("typed", "imputed", "all"),
                                         (imputed_mt.filter_rows(imputed_mt.info.TYPED | imputed_mt.info.TYPED_ONLY),
                                          imputed_mt.filter_rows(imputed_mt.info.IMPUTED),
                                          imputed_mt)):
        global_condordance, sample_concordance, variant_concordance = hl.concordance(mt, filtered_imputed_mt)
        ...
        sample_concordance.export(f"{output_prefix}.{label}.by_sample.tsv")
        ...
        variant_concordance.export(f"{output_prefix}.{label}.by_variant.tsv")
